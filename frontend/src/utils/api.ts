 ...
 * - Development: uses VITE_DEV_API_URL if set, otherwise defaults to /api/auth (same-origin)
 * - Production with VITE_API_URL set: uses VITE_API_URL
 * - Production without VITE_API_URL: /api/auth (same-origin)
 * 
 * Environment variables:
 * - VITE_DEV_API_URL: API URL for development (e.g., http://localhost:5000/api/auth)
 * - VITE_API_URL: API URL for production (e.g., https://your-backend.vercel.app/api/auth)
 */
export const getApiUrl = (): string => {
	if (import.meta.env.MODE === "development") {
		return import.meta.env.VITE_DEV_API_URL || "/api/auth";
	}
